...
function insertToDoDate() {
    //console.log('할일 추가 함수 호출!'); /* 내가 함수 잘 걸었나 확인하기위함 */
...
function changeCheckstate($label) {
    ...
    //dateId를 기반으로 배열을 탐색하여 data-id와 일치하는
...
//할 일 삭제 처리 함수 정의 //delTarget(지워질 li라는 의미로 썼음)
...
function enterModifyMode($modSpan) {
    ...
    //수정 모드 진입 버튼을 교체(Inr-undo -> Inr-checkmark-circle)
    $modSpan.classList.replace('Inr-undo', 'Inr-checkmark-circle');
    ...
    $label.replaceChild($modInput, $textSpan); //modinput이랑 textspan이랑 교체하는 것
}
...
(function() {
    ...
    $addBtn.addEventListener('click', e => {
        ...
        //form-button태그는 타입을 명확히 써주지않으면 자동 submit 발생함
        //e.preventDefault();를 써주거나 button태그 타입을 명확히 써줘야함
    ...
    $todoList.addEventListener('change', e => {
        ...
        changeCheckstate(e.target.parentNode);//label을 함수의 
    });
    ...
    $todoList.addEventListener('click', e => {
        if(e.target.matches('.todo-list .modify span.Inr-undo')) {
            enterModifyMode(e.target); //수정 모드 진입.
        } else if(e.target.matches('.todo-list .modify span.Inr-checkmark-circle')) {
